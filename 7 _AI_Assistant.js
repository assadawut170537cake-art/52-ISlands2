...
// =================================================================
// 🧠 AGENT SKILLS: AI & Workflow
// =================================================================

/**
 * 📚 Skill: literature-search-arxiv (นักสืบงานวิจัยเทคโนโลยี)
 * เรียก API ของ arXiv เพื่อหาเปเปอร์วิชาการมาช่วยอ้างอิงการแก้ปัญหา
 * @param {String} searchQuery - คำค้นหา (เช่น "machine learning construction")
 * @returns {Array} - รายการงานวิจัย
 */
function searchArxivLiterature(searchQuery) {
  try {
    const url = `http://export.arxiv.org/api/query?search_query=all:${encodeURIComponent(searchQuery)}&start=0&max_results=3`;
    const response = UrlFetchApp.fetch(url, { muteHttpExceptions: true });
    
    // หมายเหตุ: arXiv ส่งกลับเป็น XML ในระบบจริงอาจต้องใช้ XmlService ในการสกัดข้อมูล
    // ฟังก์ชันนี้เป็นโครงสร้างเตรียมรับข้อมูลสำหรับ AI
    if (response.getResponseCode() === 200) {
       return { success: true, data: "arXiv API Connected (XML Data Fetching Ready)" };
    }
    return { success: false, message: "ไม่สามารถดึงข้อมูลงานวิจัยได้" };
  } catch (error) {
...
/**
 * 🤖 Skill: ml-best-practices (ฝังเข้าไปใน System Instruction)
 * อัปเดตฟังก์ชัน getDynamicPrompt เดิมของคุณให้มีข้อนี้เพิ่มเข้าไป
 */
// นำข้อความนี้ไป "แทรกเพิ่ม" ในบรรทัด prompt ของฟังก์ชัน getDynamicPrompt เดิม:
/*
  4. กฎ ML-Best-Practices: เมื่อต้องประมวลผลข้อมูลหรือสร้างโครงสร้างโค้ด
     ให้ใช้หลักการ Clean Code, มีการดักจับข้อผิดพลาด (try-catch) เสมอ
     และจัดกลุ่มข้อมูลเพื่อลดการประมวลผลซ้ำซ้อน
*/

/**
 * ⚙️ Skill: workflow-skill-creator (เครื่องมือสร้างระบบอัตโนมัติ)
 * บันทึกแพทเทิร์นการทำงานที่สำเร็จเพื่อสร้างเป็น Template
 * @param {Object} successfulPayload - ข้อมูลที่ทำงานสำเร็จ
 */
function extractSkillFromWorkflow(successfulPayload) {
  try {
    // นำ Payload ที่สมบูรณ์ไปบันทึกลง Sheet System_Changelog 
    // เพื่อให้ AI ใช้อ้างอิงเป็น "ตัวอย่างที่ถูกต้อง" ในครั้งถัดไป
    const logData = {
      timestamp: new Date().toISOString(),
      type: "SKILL_CREATED",
      pattern: JSON.stringify(successfulPayload)
    };
    // เรียกใช้ฟังก์ชัน logAuditTrail เดิมของคุณ
    logAuditTrail("SYSTEM", "WORKFLOW_SKILL", "", JSON.stringify(logData), 1.0, "SAVE", "Pattern Extracted");
  } catch (error) {
    console.error("Workflow Skill Creator Error:", error);
  }
}
/**
 * ฟังก์ชันสำหรับประมวลผลข้อความและส่งต่อให้ AI (Gemini)
 * @param {string} userMessage - ข้อความที่ผู้ใช้พิมพ์
 * @param {string} userId - LINE User ID
 * @param {string} replyToken - โทเคนสำหรับตอบกลับ
 * @param {boolean} isAdmin - สถานะแอดมิน
 * @param {string} userState - สถานะของผู้ใช้
 * @param {object} event - ข้อมูลเหตุการณ์จาก LINE
 */
function processMessageWithAIBot(userMessage, userId, replyToken, isAdmin, userState, event) {
  try {
    // 1. ตรวจสอบเบื้องต้น
    if (!userMessage || !replyToken) return;

    // 2. เรียกใช้งาน Gemini API (อ้างอิงจาก Wrapper ที่คุณมีในโปรเจกต์)
    // หมายเหตุ: ต้องมั่นใจว่าฟังก์ชัน generateAIResponse มีอยู่จริงใน 2_gemini_wrappers.gs
    let aiResponse = "";
    if (typeof generateAIResponse === "function") {
      aiResponse = generateAIResponse(userMessage, userId);
    } else {
      aiResponse = "ขออภัยครับ ระบบ AI กำลังปรับปรุง (ไม่พบ Wrapper)";
    }

    // 3. ส่งคำตอบกลับไปยัง LINE (ต้องมีฟังก์ชันส่งกลับในไฟล์ 1_botline.gs หรือ Shared)
    // เปลี่ยนฟังก์ชันนี้ให้ตรงกับชื่อฟังก์ชันที่ใช้ส่งข้อความจริงในโปรเจกต์ของคุณ
    if (typeof sendLineReply === "function") {
      sendLineReply(replyToken, aiResponse);
    } else {
      // Fallback กรณีหาฟังก์ชันส่งข้อความไม่เจอ (ใช้ UrlFetchApp โดยตรง)
    ...
    console.log("AI Processed for " + userId + ": " + aiResponse);

  } catch (e) {
    console.error("Critical Error in processMessageWithAIBot: " + e.toString());
  }
}
/**
 * ฟังก์ชันประมวลผลข้อความผ่าน Gemini พร้อมระบบตรวจสอบและซ่อมแซม JSON อัตโนมัติ
 * @param {String} msg - ข้อความรายงานจากหน้างาน
 * @returns {Object|null} - ข้อมูล JSON ที่พร้อมบันทึก หรือ null หากไม่สามารถกู้ข้อมูลได้
 */
function processMessageWithAI(msg) {
  try {
    ...
    const systemInstruction = `คุณคือ Data Extraction Engine หน้าที่คือสกัดข้อมูลรายงานตัวพนักงานเป็น JSON เท่านั้น
    กฎ:
    1. ห้ามมีข้อความเกริ่นนำเด็ดขาด
    2. ต้องเป็น JSON ที่ถูกต้องสมบูรณ์
    3. หากพบชื่อพนักงาน ให้ลบคำนำหน้าชื่อออก (นาย, นาง, น.ส.)
    4. โครงสร้าง: { "date": "DD/MM/YYYY", "default_site": "...", "time_start": "08.00", "time_end": "17.00", "employees": [{"firstname": "...", "lastname": "...", "task": "..."}] }`;

    const payload = {
      contents: [{ parts: [{ text: "รายงาน: " + msg }] }],
    ...
    // 🔥 ระบบซ่อม JSON อัจฉริยะ: ตรวจสอบและตัดส่วนเกิน (เช่น markdown ```json)
