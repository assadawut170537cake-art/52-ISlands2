...
function doPost(e) {
  ...
  try {
    ...
    if (requestData.events && requestData.events.length > 0) {
      ...
      // ⚡ ปล่อยให้ handleLineWebhook ทำหน้าที่คัดกรองอย่างละเอียดและตอบกลับ
...
function handleLineWebhook(event) {
  try {
    ...
    if (!message || message.type !== "text") {
      try {
        emergencyReply(replyToken, "⚠️ ระบบรองรับเฉพาะข้อความและรูปภาพบัตรตอกเท่านั้นครับ");
      } catch(e) {
        console.error("Fallback Reply Error: " + e.message);
      }
      return;
    }
    ...
    let pendingSuggestionsRaw = cache.get(`PENDING_SUGGESTIONS_${userId}`);

    // ตัดเครื่องหมาย # ออกเพื่อสกัดเป็นคำสั่ง (Command Parsing) ไว้ที่จุดเริ่มต้น
    let commandText = msg;
    if (msg.startsWith("#")) {
      commandText = msg.substring(1).trim();
    }

    // กำหนดลอจิกข้อยกเว้น: หากผู้ใช้พิมพ์คำที่ตรงกับปุ่มตอบกลับ ให้ถือว่าไม่ใช่คำสั่งเริ่มใหม่แม้จะมี # นำหน้า
    // เปลี่ยนมาใช้ includes() เพื่อความยืดหยุ่นหากผู้ใช้พิมพ์ข้อความนำหน้าหรือต่อท้าย
    const isOtReply = commandText.includes("ทำที่เดิม") || commandText.includes("เปลี่ยนไซต์");

    // หากขึ้นต้นด้วย # แสดงว่าเริ่มคำสั่งใหม่ (และไม่ใช่การตอบปุ่ม OT) ให้เคลียร์สถานะเดิมทิ้งทั้งหมด
    if (msg.startsWith("#") && !isOtReply && (pendingClockIn12 || pendingOTConfirm || pendingOTDetails || pendingSuggestionsRaw)) {
      ...
      // อัปเดตตัวแปรเพื่อไม่ให้รันในบล็อกถัดไป
      pendingClockIn12 = null;
      pendingOTConfirm = null;
      pendingOTDetails = null;
      pendingSuggestionsRaw = null;
    ...
    if (commandText === "ยกเลิก" || commandText === "ถ่ายใหม่" || commandText === "ยกเลิกลงเวลา") {
      cache.remove(`PENDING_CLOCKIN_${userId}`);
      cache.remove(`PENDING_OT_CONFIRM_${userId}`);
      cache.remove(`PENDING_OT_DETAILS_${userId}`);
    ...
    // [STATE]: แจ้งเตือนเซสชันหมดอายุ (Timeout Fallback)
    // หากผู้ใช้พิมพ์ปุ่มตอบกลับของ OT แต่ CacheService หมดอายุไปแล้ว (pendingOTConfirm เป็น null)
    if (isOtReply && (!pendingOTConfirm || pendingOTConfirm === "CLEARED")) {
      console.error(`Cache Timeout: User ${userId} replied '${commandText}' but PENDING_OT_CONFIRM expired.`);
      emergencyReply(replyToken, "⚠️ เซสชันการทำรายการโอทีของคุณหมดอายุแล้ว (เกินเวลาที่กำหนด)\nกรุณาเริ่มทำรายการใหม่อีกครั้งครับ");
      return;
    }

    // [STATE]: จัดการการยืนยันรายละเอียดไซต์งานโอที
    if (pendingOTDetails && pendingOTDetails !== "CLEARED") {
      // ป้องกันการทับซ้อนกับการกดปุ่มเปลี่ยนไซต์อีกรอบ
      if (isOtReply) {
        emergencyReply(replyToken, "⚠️ คุณได้เลือกเปลี่ยนไซต์ไปแล้ว กรุณาพิมพ์ระบุ 'ไซต์งาน / งานที่ทำ' แทนการกดปุ่มครับ");
        return;
      }
      ...
      try {
        ...
        if (typeof logAuditTrail === "function") {
          logAuditTrail(userId, "PROCESS_OT_DETAILS", commandText, JSON.stringify(dataToProcess), 1.0, "ACCEPT_DETAILS", "ประมวลผลรายละเอียดไซต์งานโอที");
        }
        
        finalizeClockInSaving(dataToProcess, userId, replyToken, dataToProcess.checkStatus, commandText, dataToProcess.targetFileId);
      } catch (error) {
        console.error("Error processing PENDING_OT_DETAILS: " + error.message);
        emergencyReply(replyToken, "❌ เกิดข้อผิดพลาดในการประมวลผลรายละเอียดไซต์โอที กรุณาลองใหม่อีกครั้ง");
      }
      return;
    }
    ...
    if (pendingOTConfirm && pendingOTConfirm !== "CLEARED") {
      try {
        ...
        // ใช้ commandText แทน msg และตรวจจับด้วย includes เพื่อความยืดหยุ่น
        if (commandText === "ทำที่เดิม/งานเดิม" || commandText.includes("ทำที่เดิม")) {
          cache.remove(`PENDING_OT_CONFIRM_${userId}`);
          
          if (typeof logAuditTrail === "function") {
            logAuditTrail(userId, "PROCESS_OT_CONFIRM", commandText, JSON.stringify(dataToProcess), 1.0, "SAME_SITE", "ยืนยันการทำโอทีที่เดิม");
          }
          
          finalizeClockInSaving(dataToProcess, userId, replyToken, dataToProcess.checkStatus, null, dataToProcess.targetFileId);
          return;
          
        } else if (commandText === "เปลี่ยนไซต์/เปลี่ยนงาน" || commandText.includes("เปลี่ยนไซต์")) {
          cache.remove(`PENDING_OT_CONFIRM_${userId}`);
          cache.put(`PENDING_OT_DETAILS_${userId}`, JSON.stringify(dataToProcess), 300); // ตั้งเวลาใหม่ 5 นาที (300 วินาที)
          
          if (typeof logAuditTrail === "function") {
            logAuditTrail(userId, "PROCESS_OT_CONFIRM", commandText, JSON.stringify(dataToProcess), 1.0, "CHANGE_SITE", "ร้องขอเปลี่ยนไซต์ทำโอที");
          }
          ...
          // หากพิมพ์อย่างอื่นมาโดยที่ยังไม่กดปุ่ม
          emergencyReply(replyToken, "⚠️ กรุณากดเลือกปุ่ม 'ทำที่เดิม/งานเดิม' หรือ 'เปลี่ยนไซต์/เปลี่ยนงาน' เพื่อดำเนินการต่อครับ");
          return;
        }
      } catch (error) {
        console.error("Error processing PENDING_OT_CONFIRM: " + error.message);
        try {
          emergencyReply(replyToken, "❌ เกิดข้อผิดพลาดในการประมวลผลการยืนยันโอที กรุณาลองใหม่อีกครั้ง");
        } catch(replyErr) {
    ...
    if (pendingClockIn12 && pendingClockIn12 !== "CLEARED") {
      if (commandText === "ยืนยันตามเวลาที่แจ้ง" || commandText === "ลงเวลา 13.00 น.") {
        processPendingClockIn(commandText, pendingClockIn12, userId, replyToken);
        if (typeof logAuditTrail === "function") logAuditTrail(userId, "PROCESS_CLOCKIN_12", commandText, pendingClockIn12, 1.0, "CONFIRM_12", "ยืนยันเวลาทำงานช่วงบ่าย");
        return;
      }
    }
    ...
    if (systemStatus === "OFF" && !isUserAdmin) {
      try {
        emergencyReply(replyToken, "🔴 ขณะนี้ระบบปิดรับงานชั่วคราวครับ (System Offline)");
      } catch(e) {
        console.error("Fallback Reply Error: " + e.message);
      }
      return;
...
function finalizeClockInSaving(data, userId, token, check, customOt, targetId) {
  try {
    ...
    if (customOt && writeRes.count > 0) {
      ...
      try {
        const ss = writeRes.spreadsheet || (typeof getCachedSpreadsheet === "function" ? getCachedSpreadsheet(targetId) : SpreadsheetApp.openById(targetId));
    ...
    if (!data.time_end || data.time_end === "") {
        ...
        let otText = otHr > 0 ? `+${otHr}ชม.` : "";
        timeStatus = `(เวลา: ${data.time_start}-${data.time_end}) ${normHr} ชม.${otText}`;
    }
    ...
    // ตรวจสอบ Error และประกอบร่างข้อความส่งกลับ
    if (writeRes && writeRes.errors && writeRes.errors.length > 0) txt += `⚠️ หาชื่อไม่พบ: ${writeRes.errors.join(', ')}\n`;

    const finalMessage = `${(writeRes && writeRes.count === 0) || countValue === 0 ? "❌ ไม่พบข้อมูล" : `✅ บันทึกสำเร็จ ${countValue} คน`}
${data.date ? `📅 วันที่: ${data.date}` : ""}
${timeStatus}
ไซต์: ${displaySite}
[ที่พัก: ${accomText}]
${isTesting ? "🧪 [โหมดทดสอบ]" : ""}
${txt ? txt : ""}📌 โปรดตรวจเช็คความถูกต้อง หากผิดพลาดแจ้งแอดมินทันที`;

    // ส่งข้อความกลับเข้ากลุ่ม
    emergencyReply(token, finalMessage.trim());
