 ...
 * @description ประมวลผลรูปภาพร่วมกับ Gemini API และแปลงผลลัพธ์เป็นโครงสร้างข้อมูล JSON แบบปลอดภัยด้วย Exponential Backoff
 * @param {string} base64Str - ข้อมูลรูปภาพในรูปแบบ Base64 String
 * @param {string} system - คำสั่งควบคุมระบบ (System Instruction)
 * @param {string} mimeType - ประเภทของไฟล์รูปภาพ เช่น image/jpeg, image/png
 * @returns {Object} ข้อมูล JSON ผลลัพธ์จากการสกัดข้อมูลพนักงาน
 */
async function callGeminiVision(base64Str, system, mimeType) {
  /* [try-catch block สำหรับ Error Handling เชิงลึกในระบบเครือข่าย] */
  try {
    const modelName = typeof getDynamicConfig === 'function' ? getDynamicConfig('MODEL_NAME') : 'gemini-1.5-flash';
    const apiKey = typeof getDynamicConfig === 'function' ? getDynamicConfig('GEMINI_API_KEY') : '';
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${modelName}:generateContent?key=${apiKey}`;
    
    const payload = { 
      contents: [{ parts: [{ text: "Extract worker codes." }, { inlineData: { mimeType: mimeType, data: base64Str } }] }], 
    ...
    return withExponentialBackoff(() => {
      ...
      throw new Error("พอร์ตการเชื่อมต่อดึงข้อมูลจาก Gemini Vision ปฏิเสธการตอบกลับ: " + res.getResponseCode());
    });
  } catch (error) {
    Logger.log("เกิดข้อผิดพลาดในฟังก์ชัน callGeminiVision: " + error.toString());
    throw error;
  }
}
 ...
 * @description ใช้ Gemini API แปลงข้อความสไตล์คำสั่งงานทั่วไปให้เป็นโครงสร้างออบเจกต์ JSON มาตรฐานเพื่อส่งเข้า Database
 * @param {string} message - ข้อความภาษาไทยที่ต้องการให้ AI วิเคราะห์โครงสร้าง
 * @returns {Object} JSON Object ที่แปลงตามโครงสร้างแกนหลักแล้ว
 */
async function processMessageWithAI(message) {
  try {
    ...
    Logger.log("เกิดข้อผิดพลาดในฟังก์ชัน processMessageWithAI: " + error.toString());
    return null;
  }
}
...
// =================================================================
// 🛠️ 4. Helpers จัดการระบบและดึงข้อมูลฐานพนักงานภายนอก
// =================================================================

/**
 * @function normalize
 * @description คัดกรองตัวอักษร ตัดคำนำหน้าชื่อไทยและลบช่องว่างออกเพื่อเพิ่มความเที่ยงตรงในการจับคู่คำ
 * @param {string} t - ชื่อที่ต้องการจัดระเบียบข้อมูล
 * @returns {string} ข้อความสะอาดที่พร้อมนำไปวิเคราะห์
 */
function normalize(t) { 
  return t ? t.toString().replace(/^(นาย|นาง|น\.?ส\.?|ด\.?ช\.?|ด\.?ญ\.?)/g, "").replace(/\s/g, "") : ""; 
}

/**
 * @function formatDateToShort
 * @description ปรับแต่งฟอร์แมตข้อมูล Date Object เป็นสตริงวันที่แบบสั้น
 * @param {Date} dateObj - ตัวแปรประเภท Date
 * @returns {string|Object} วันที่แบบย่อ
 */
