...
function parseComplexMessage(text) {
  try {
    ...
    // 🛠️ จุดที่เพิ่มเข้ามาใหม่: หากช่างไม่พิมพ์ "1. ชื่อ" (ทำให้ employees ว่างเปล่า) ให้สแกนข้อความแบบยืดหยุ่น
    if (employees.length === 0 && typeof extractEmployeesFromText === 'function') {
        employees = extractEmployeesFromText(text);
        
        // แนบที่พักและ OT เที่ยง (ถ้ามี) ให้พนักงานทุกคนที่สแกนเจอ
...
// ฟังก์ชันด่านตรวจชื่อ
function getVerifiedRowIndex(inputName, dbData) {
    ...
    for (let i = 0; i < dbData.length; i++) {
        ...
        if (rowName === normInput) return { index: 3 + i, status: "ok" };
        
        // ถ้าเจอคำที่ซ่อนอยู่ (เช่น พิมพ์ ออง เจอ อองจี) ให้เก็บไว้พิจารณา
        if (rowName.includes(normInput)) {
            multipleMatches.push(rowName);
        }
    }

    if (multipleMatches.length > 0) {
        return { index: -1, status: "warning", matches: multipleMatches };
    }
    return { index: -1, status: "error" };
}

// =================================================================
// 🛡️ 1. ระบบจัดการฐานข้อมูลผ่าน ChatOps (แก้ชื่อ/รหัส/ลางาน/คนขาด)
// =================================================================

function updateEmployeeDatabase(oldName, newName, newEmpId) {
    try {
        const ssId = getDynamicConfig("EXTERNAL_DATABASE_ID");
        const sheet = SpreadsheetApp.openById(ssId).getSheetByName(getDynamicConfig("DATABASE_SHEET_NAME") || "รายชื่อพนักงาน");
        const data = sheet.getDataRange().getValues();
        let updated = false;

        for (let i = 1; i < data.length; i++) {
            if (normalize(data[i][2]) === normalize(oldName)) { // คอลัมน์ C (Index 2) คือชื่อ
                if (newName) sheet.getRange(i + 1, 3).setValue(newName);
                if (newEmpId) sheet.getRange(i + 1, 14).setValue(newEmpId); // คอลัมน์ N (Index 13) คือรหัส
                updated = true;
                break;
            }
        }
        return updated ? `✅ อัปเดตข้อมูลของ '${oldName}' สำเร็จ!` : `❌ ไม่พบพนักงานชื่อ '${oldName}' ในฐานข้อมูล`;
    } catch (e) { return `❌ Error: ${e.message}`; }
}

function handleCheckAbsent(dateStr) {
    try {
        const fileId = getTargetFileIdByDate(dateStr);
        if (!fileId) return "❌ ไม่พบไฟล์บันทึกเวลาของเดือนนี้";
        
        const ss = SpreadsheetApp.openById(fileId);
        const dailySheet = ss.getSheetByName(parseThaiDate(dateStr));
        if (!dailySheet) return `❌ ยังไม่มีการลงเวลาในวันที่ ${dateStr} ครับ`;

        // 1. ดึงชื่อคนที่มาทำงานแล้ว
        const presentData = dailySheet.getRange(3, 4, dailySheet.getLastRow(), 1).getValues();
        const presentNames = presentData.map(r => normalize(r[0])).filter(Boolean);

        // 2. ดึงชื่อพนักงานทั้งหมดที่ Active จากฐานข้อมูล
        const activeEmps = getEmployeeListFromSheet();

        // 3. หาคนขาด (มีใน DB แต่ไม่มีใน Daily Sheet)
        const absents = activeEmps.filter(emp => !presentNames.includes(normalize(emp)));

        if (absents.length === 0) return `✅ วันที่ ${dateStr} มาทำงานครบทุกคนครับ!`;
        return `📊 รายงานคนขาดวันที่ ${dateStr}:\nรวม ${absents.length} คน ได้แก่\n- ` + absents.join("\n- ");
    } catch (e) { return "❌ Error: " + e.message; }
}

function recordLeaveData(name, type, dateStr) {
    try {
        const ssId = getDynamicConfig("EXTERNAL_DATABASE_ID");
        let sheet = SpreadsheetApp.openById(ssId).getSheetByName(getDynamicConfig("LEAVE_SHEET_NAME") || "ข้อมูลการลา");
        if (!sheet) return "❌ ไม่พบชีต 'ข้อมูลการลา' ในไฟล์ฐานข้อมูลหลัก";
        
        const timestamp = Utilities.formatDate(new Date(), "GMT+7", "dd/MM/yyyy HH:mm:ss");
        sheet.appendRow([timestamp, name, type, dateStr, "รออนุมัติ"]);
        return `✅ บันทึก '${name}' ลา${type} วันที่ ${dateStr} เรียบร้อยแล้ว`;
    } catch (e) { return "❌ Error: " + e.message; }
}

// =================================================================
// 🔍 2. ระบบสแกนค้นหารายชื่อแบบยืดหยุ่น (Flexible Name Matching)
// =================================================================
...
function getEmployeeListFromSheet() {
    try {
        ...
        const sheet = SpreadsheetApp.openById(ssId).getSheetByName(getDynamicConfig("DATABASE_SHEET_NAME") || "รายชื่อพนักงาน");
        const data = sheet.getDataRange().getValues();
        // ดึงเฉพาะคนที่มีสถานะ "ทำงาน" หรือ "ปกติ" (สมมติอยู่คอลัมน์ K / Index 10)
        return data.slice(1).filter(r => r[10] === "ทำงาน" || r[10] === "ปกติ").map(r => r[2].toString().trim());
    } catch (e) { return []; }
}
...
function extractEmployeesFromText(messageText) {
    const activeEmps = getEmployeeListFromSheet();
    let foundEmployees = [];
    const normMsg = messageText.replace(/\s+/g, ""); // ตัดช่องว่างออกให้หมดเพื่อสแกน

    activeEmps.forEach(empName => {
        const normEmp = normalize(empName);
        if (normEmp && normMsg.includes(normEmp)) {
            // ถ้าเจอชื่อแทรกอยู่ในข้อความ ให้สร้าง Object เตรียมส่งไปบันทึก
            foundEmployees.push({
                firstname: empName,
                lastname: "",
                task: "ทำงาน", 
